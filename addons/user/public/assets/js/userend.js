define(['fast', 'template', 'moment'], function (Fast, Template, Moment) {
    var Userend = {
        ...
        init: function () {
            ...
            $(document).on('click', '.btn-dialog,.dialogit', function (e) {
                ...
                var url = Userend.api.replaceids(that, $(that).attr('href'));
                var title = $(that).attr("title") || $(that).data("title") || $(that).data('original-title');
                var button = Userend.api.gettablecolumnbutton(options);
                if (button && typeof button.callback === 'function') {
                    options.callback = button.callback;
                }
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
                        Userend.api.open(url, title, options);
                        Layer.close(index);
                    });
                } else {
                    Userend.api.open(url, title, options);
            ...
            $(document).on('click', '.btn-ajax,.ajaxit', function (e) {
                ...
                options.url = Userend.api.replaceids(this, options.url);
                ...
                if (typeof options.confirm !== 'undefined') {
                    Layer.confirm(options.confirm, function (index) {
                        Userend.api.ajax(options, success, error);
                        Layer.close(index);
                    });
                } else {
                    Userend.api.ajax(options, success, error);
    ...
    Userend.api = $.extend(Fast.api, Userend.api);
    //将Template渲染至全局,以便于在子框架中调用
    window.Template = Template;
    //将Moment渲染至全局,以便于在子框架中调用
    window.Moment = Moment;
    //将Userend渲染至全局,以便于在子框架中调用
    window.Userend = Userend;
