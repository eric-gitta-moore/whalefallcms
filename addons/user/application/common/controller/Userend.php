 ...
 * 用户端控制器基类
 */
class Userend extends Controller
{
    /**
     * 无需登录的方法,同时也就不需要鉴权了
     * @var array
     ...
     * 引入会员控制器的traits
     */
    use \app\common\library\traits\Userend;
    ...
    protected function buildparams($searchfields = null, $relationSearch = null, $aliasName = null)
    {
        ...
        if (is_array($userIds)) {
            $where[] = [$tableName . $this->dataLimitField, 'in', $userIds];
        }
        if ($search) {
