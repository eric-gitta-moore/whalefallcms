...
class Hyperdown
{
     ...
     * @var string
     */
    public $_commonAttrWhiteList = 'title|src|href|id|class|style|width|height|alt|target|align|data-type|data-id|data-toggle|data-title';
    /**
     * html tags
     *
     * @var string
    ...
    public function makeHolder($str)
    {
        ...
        $str = $this->stripAttributes($str, explode('|', $this->_commonAttrWhiteList));
        $this->_id++;
     ...
     * @return mixed
     */
    function stripAttributes($str, $allowedattr = array())
    {
        if (preg_match_all("/<[^>]*\\s([^>]*)\\/*>/msiU", $str, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                $tag = $match[0];
                $attrs = array();
                preg_match_all("/\\s.*=(['\"]).*\\1/msiU", " " . $match[1], $split, PREG_SET_ORDER);
                ...
                $rpl = str_replace($match[1], $attrs, $tag);
